import{s as n,o as A,p as H,j as e,a as f,e as J,q as me,h as C,c as ue,d as ge,F as Y,g as R,u as S,t as je,v as fe,_ as y,w as we,r as b,x as X,y as ye,z as be,A as Ae,B as $,D as Fe,E as Ce,C as Re}from"./index-aa4cd388.js";import{a as D,W as G,I as u,b as v,S as Z,c as j,d as p,B as W,H as K,e as m,f as T,g as _,h as U,i as Se,j as z,k as L}from"./Components.styled-81838d60.js";import{M as ee,s as M,d as De,b as te,l as ne,a as ie,c as re}from"./formikDiary-3a6b9faa.js";import{C as se,A as ae,D as oe}from"./index-8361112c.js";import{g as ke,a as ve,R as Qe,F as Be,b as Ie,c as Pe,d as Ee}from"./getRecommendedFood-edd977f9.js";const We=n.main`
  padding-top: 16px;
  padding-bottom: 80px;
  @media screen and (min-width: 834px) {
    padding-top: 24px;
    padding-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    padding-top: 20px;
    padding-bottom: 54px;
  }
`,Ne=n.div`
  display: flex;
  flex-wrap: nowrap;
  align-items: center;
  justify-content: space-between;

  margin-bottom: 20px;

  @media screen and (min-width: 834px) {
    margin-bottom: 24px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 16px;
  }
`,Oe=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  margin-bottom: 80px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    flex-wrap: wrap;
    justify-content: space-between;
    column-gap: 20px;
    margin-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 54px;
  }
`,Ge=n.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 328px;
  }
`,Ke=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  @media screen and (min-width: 834px) {
    row-gap: 32px;
  }
  & li {
    display: flex;
    flex-direction: row;
    align-items: center;
    column-gap: 20px;
  }
`;function Te(){const{baseWater:t,BMR:r}=A(H),s=t!==null?t*1e3:0;return e.jsxs(Ge,{children:[e.jsx(D,{children:"Daily goal"}),e.jsx(G,{children:e.jsxs(Ke,{children:[e.jsxs("li",{children:[e.jsx(u,{width:"80px",height:"80px",stroke:"var(--green)",children:e.jsx("use",{href:`${f}#bubble`})}),e.jsxs("div",{children:[e.jsx(v,{color:"var(--grey-light)",children:"Calories"}),e.jsx(Z,{children:r||0})]})]}),e.jsxs("li",{children:[e.jsx(u,{width:"80px",height:"80px",stroke:"var(--violet)",children:e.jsx("use",{href:`${f}#water`})}),e.jsxs("div",{children:[e.jsx(v,{color:"var(--grey-light)",children:"Water"}),e.jsxs(j,{children:[e.jsx(Z,{children:s||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]})]})]})]})})]})}const Ue=n.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 444px;
  }
`,ze=n(G)`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 20px;

  @media screen and (min-width: 834px) {
    column-gap: 40px;
  }
`,Le=n(v)`
  margin-top: 13px;
`,Me=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 8px;
  margin-top: 12px;
  margin-bottom: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 20px;
  }
`,qe=n.div`
  width: 80px;
  height: 192px;
  padding: 8px;
  border-radius: 20px;
  background-color: var(--black-primary);
  border: 1px solid var(--grey);
  display: flex;
  flex-direction: column;
  justify-content: flex-end;
`,Ze=n.p`
  position: absolute;
  top: ${t=>t.$top};
  left: 50%;
  transform: translateX(-50%);

  color: ${t=>t.$color};

  text-align: center;
  font-size: 16px;
  font-weight: 600;
  line-height: 1.38;
`,Ve=n.div`
  position: relative;

  background-color: var(--violet);
  border-radius: 20px;
  height: ${t=>t.height||"5%"};
`,He=n.div`
  flex-grow: 1;
  display: flex;
  flex-direction: column;

  & > button:nth-child(1) {
    align-self: flex-end;
  }
`,Je=n.div`
  margin: 16px 12px 22px;

  @media screen and (min-width: 400px) {
    margin: 24px 40px;
    max-width: 338px;
  }
`,ce=n(W)`
  background-color: transparent;
  color: var(--grey-light);
  width: 100%;
  margin-top: 8px;
  padding-bottom: 0;
  &:hover {
    box-shadow: none;
  }
`,Ye=n(J)`
  @media screen and (min-width: 834px) {
    max-width: 212px;
    margin: 0 auto;
  }
`,Xe=n(me)`
  margin-top: 24px;
`,$e=n(C)`
  & + p {
    margin-top: 12px;
  }
  & + button {
    margin-top: 32px;
  }
`,_e=n(W)`
  width: 100%;
`,et=ue().shape({water:ge().min(100,"A quantity of water must be at least 100 ml").max(5e3,"A quantity of water must be at least 5000 ml").required("A quantity of water is required").typeError("A quantity must be a number")});function tt({handleUpdate:t,setModalActive:r}){const s=o=>{t(o)};return e.jsxs(Je,{children:[e.jsx(K,{children:"Add water intake"}),e.jsx(Y,{initialValues:{water:""},validationSchema:et,onSubmit:s,children:({errors:o,touched:a,isSubmitting:h,isValid:x,dirty:d})=>e.jsxs(Ye,{children:[e.jsx(Xe,{children:e.jsx(m,{children:"How much water"})}),e.jsx($e,{type:"text",name:"water",placeholder:"Enter milliliters",autoComplete:"off",className:a.water?o.water?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"water"}),e.jsx(_e,{type:"submit",disabled:!x||h||!d,children:e.jsx(T,{children:"Confirm"})})]})}),e.jsx(ce,{type:"button",onClick:()=>r(!1),children:e.jsx(_,{children:"Cancel"})})]})}const nt=t=>t.water;function it({modalActive:t,setModalActive:r}){const s=S(),{water:o=0,error:a}=A(nt),h=A(je),x=h!==null?h*1e3:0,d=x-o,l=Math.max(0,d),i=Math.round(o*100/x),c=async w=>{r(!1);try{await s(fe({water:+w.water})).unwrap(),y.success("Added to the total amount of water drunk for today!")}catch(F){a&&y.error(`${a}`),y.error(`${F}`)}},g=async()=>{try{await s(we()).unwrap(),y.success("You have successfully remove daily water!")}catch(w){y.error(`${w}`)}};return e.jsxs(Ue,{children:[e.jsx(D,{children:"Water"}),e.jsxs(ze,{children:[e.jsx(qe,{children:e.jsx(Ve,{height:`${i}%`,children:e.jsxs(Ze,{$color:i<84?"var(--violet)":"var(--black-primary)",$top:i<84?"-28px":"28px",children:[i,"%"]})})}),e.jsxs(He,{children:[e.jsx(U,{type:"button",onClick:()=>g(),children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${f}#trash`})})}),e.jsx(Le,{children:"Water consumption"}),e.jsxs(Me,{children:[e.jsxs(j,{children:[e.jsx(Se,{children:o||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]}),e.jsxs(j,{children:[e.jsx(p,{children:"left:"}),e.jsx(m,{children:l||0}),e.jsx(p,{color:"var(--grey-light)",children:"ml"})]})]}),e.jsxs(W,{type:"button",onClick:()=>r(!0),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${f}#add`})}),e.jsx(T,{children:"Add water intake"})]})]})]}),t&&e.jsx(ee,{modalActive:t,setModalActive:r,children:e.jsx(tt,{handleUpdate:c,setModalActive:r})})]})}const rt=t=>{function r(o){const a={calories:0,carbohydrate:0,protein:0,fat:0};return o.forEach(({calories:h,carbohydrate:x,protein:d,fat:l})=>{a.calories+=h,a.carbohydrate+=x,a.protein+=d,a.fat+=l}),{calories:a.calories,carbohydrate:a.carbohydrate,protein:a.protein,fat:a.fat}}return Object.keys(t).reduce((o,a)=>(Array.isArray(t[a])&&o.push({mealType:a,...r(t[a])}),o),[])},st=(t={})=>{const r={calories:0,carbohydrate:0,protein:0,fat:0};return Object.values(t).forEach(s=>{Array.isArray(s)&&s.forEach(({calories:o,carbohydrate:a,protein:h,fat:x})=>{r.calories+=o,r.carbohydrate+=a,r.protein+=h,r.fat+=x})}),r},at=n.li`
  min-width: 100%;
  @media screen and (min-width: 1440px) {
    min-width: 560px;
  }
`,ot=n(G)`
  display: flex;
  flex-direction: column;
  row-gap: 12px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 40px;
    padding: 36px 40px;
  }
`,ct=n.ul``;se.register(ae);const lt=({calories:t,baseCalories:r})=>{const s=b.useMemo(()=>t>=r?100:Math.round(t/r*100),[t,r]),o=t>=r?"red":"rgba(69, 255, 188, 1)",a=t>=r?[0]:[50],h={datasets:[{data:[s,100-s],backgroundColor:[o,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:a,cutout:"80%"}]},x={id:"textCenter",beforeDatasetsDraw(l){const{ctx:i}=l,c=l.getDatasetMeta(0).data[0].x,g=l.getDatasetMeta(0).data[0].y;i.save(),i.font="32px Poppins",i.fillStyle="#ffffff",i.textAlign="center",i.textBaseline="middle",i.fillText(t,c,g-12),i.font="14px sans-serif",i.fillStyle="#B6B6B6",i.fillText("calories",c,g+15)}},d={id:"backgroundCircle",beforeDatasetsDraw(l){const{ctx:i}=l;i.save();const c=l.getDatasetMeta(0).data[0].x,g=l.getDatasetMeta(0).data[0].y,w=l.getDatasetMeta(0).data[0].innerRadius,F=l.getDatasetMeta(0).data[0].outerRadius,k=F-w,Q=Math.PI/180;i.beginPath(),i.lineWidth=k,i.strokeStyle="rgba(41, 41, 40, 1)",i.arc(c,g,F-k/2,0,Q*360,!1),i.stroke()}};return e.jsx("div",{className:"doughnut",style:{height:168,width:168},children:e.jsx(oe,{plugins:[x,d],data:h})})},dt=lt,O=n.li`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,xt=n.ul`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,ht=n(O)`
  &:not(:last-child) {
    margin-bottom: 12px;
  }
`;se.register(ae);const pt=({title:t,arcColor:r,nutrient:s,baseNutrient:o=0})=>{const a=b.useMemo(()=>s>=o?100:Math.round(s/o*100),[o,s]),h=s>=o?[0]:[50],x={datasets:[{data:[a,100-a],backgroundColor:[r,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:h,cutout:"80%"}]},d={id:"textCenter",beforeDatasetsDraw(i){const{ctx:c}=i,g=i.getDatasetMeta(0).data[0].x,w=i.getDatasetMeta(0).data[0].y;c.save(),c.font="14px Poppins",c.fillStyle="#B6B6B6",c.textAlign="center",c.textBaseline="middle",c.fillText(`${a}%`,g,w)}},l={id:"backgroundCircle",beforeDatasetsDraw(i){const{ctx:c}=i;c.save();const g=i.getDatasetMeta(0).data[0].x,w=i.getDatasetMeta(0).data[0].y,F=i.getDatasetMeta(0).data[0].innerRadius,k=i.getDatasetMeta(0).data[0].outerRadius,Q=k-F,pe=Math.PI/180;c.beginPath(),c.lineWidth=Q,c.strokeStyle="rgba(41, 41, 40, 1)",c.arc(g,w,k-Q/2,0,pe*360,!1),c.stroke()}};return e.jsxs(ht,{children:[e.jsx("div",{className:"doughnut",style:{height:48,width:48},children:e.jsx(oe,{plugins:[d,l],data:x})}),e.jsxs("div",{children:[e.jsx(v,{children:t}),e.jsxs(xt,{children:[e.jsxs(O,{children:[e.jsx(p,{children:"Goal:"}),e.jsx(m,{children:o||0})]}),e.jsxs(O,{children:[e.jsx(p,{children:"left:"}),e.jsx(m,{children:s>=o?0:o-s})]})]})]})]})},N=pt;function mt(){S();const{fat:t,protein:r,carbohydrate:s,BMR:o}=A(H),{meals:a,isLoading:h}=A(M),{fat:x,protein:d,carbohydrate:l,calories:i}=st(a),c=["Carbonohidrates","Protein","Fat"];return e.jsxs(at,{children:[e.jsx(D,{children:"Food"}),h?e.jsx(X,{}):e.jsxs(ot,{children:[e.jsx(dt,{calories:i,baseCalories:o}),e.jsxs(ct,{children:[e.jsx(N,{title:c[0],arcColor:"#FFC4F7",nutrient:l,baseNutrient:s}),e.jsx(N,{title:c[1],arcColor:"#FFF3B7",nutrient:d,baseNutrient:r}),e.jsx(N,{title:c[2],arcColor:"#B6B6B6",nutrient:x,baseNutrient:t})]})]})]})}const ut=n.li`
  width: 100%;

  @media screen and (min-width: 1440px) {
    width: 558px;
  }
`,gt=n.div`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  column-gap: 16px;
  justify-content: flex-start;
  align-items: center;

  margin-bottom: 16px;
`;n(u)`
  width: 36px;
  height: 36px;
  /* fill: none;
  stroke: none; */
  display: inline-block;

  stroke-width: 0;
  stroke: currentColor;
  fill: currentColor;
`;const jt=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 28px;
`,B="data:image/png;base64,",I="data:image/png;base64,",P="data:image/png;base64,",E="data:image/png;base64,",ft=n.li`
  display: flex;
  flex-direction: column;
  row-gap: 8px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    align-items: center;
    row-gap: 28px;
  }
`,wt=n.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-right: 8px;
`,yt=n(v)`
  text-transform: capitalize;
  width: 100px;
`,le=n.div`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  column-gap: 20px;
  row-gap: 12px;
  align-items: center;
  justify-content: flex-start;

  & div:nth-child(1) {
    width: 164px;
  }
  & div:nth-child(2) {
    width: 92px;
  }
  & div:nth-child(3) {
    width: 66px;
  }
`,de=n.button`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  align-items: center;
  color: var(--green-light);
  border: none;
  padding: 0;
`,xe=n(T)`
  color: currentColor;
  margin-left: 6px;
`,bt=n.div`
  min-height: 346px;
  margin: 16px 12px 22px;

  @media screen and (min-width: 834px) {
    margin: 24px;
    min-width: 630px;
  }
`,At=n.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-top: 24px;
  margin-bottom: 16px;
`,Ft=n(D)`
  text-transform: capitalize;
  width: 100px;
`,Ct=n.ul`
  display: flex;
  flex-direction: column;
  row-gap: 18px;
  align-items: flex-start;
  justify-content: flex-start;
`,Rt=n.li``,St=n(p)`
  margin-bottom: 12px;
  text-transform: capitalize;
  color: var(--green-light);
`,V=n.div`
  margin-top: 40px;
  margin-bottom: 40px;
`,Dt=n(J)`
  display: flex;
  flex-direction: column;

  row-gap: 16px;
  min-width: 276px;

  @media screen and (min-width: 834px) {
    min-width: 630px;
    align-items: flex-end;
  }
`,kt=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 12px;
  }
`,q=n.div`
  width: 100%;
  display: flex;
  flex-direction: column;
`,vt=n(q)`
  @media screen and (min-width: 834px) {
    width: 255px;
  }
`,Qt=n(q)`
  @media screen and (min-width: 834px) {
    width: 100px;
  }
`,Bt=n(q)`
  @media screen and (min-width: 834px) {
    width: 96px;
  }
`,It=n.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 12px;

  @media screen and (min-width: 834px) {
    width: auto;
  }
`,he=n.div`
  display: flex;
  flex-direction: column;
  width: calc((100% - 12px) / 2);

  @media screen and (min-width: 834px) {
    width: 60px;
  }
`,Pt=n(he)``,Et=n.div`
  display: flex;
  flex-direction: column;
  row-gap: 12px;
  width: 100%;
  margin-top: 60px;
  margin-bottom: 60px;

  @media screen and (min-width: 834px) {
    flex-direction: row-reverse;
    column-gap: 12px;
    width: 416px;
    justify-content: flex-end;
  }
`,Wt=n(W)`
  width: 100%;
  @media screen and (min-width: 834px) {
    width: calc(100% / 2 - 12px);
  }
`,Nt=n(ce)`
  width: 100%;
  @media screen and (min-width: 834px) {
    width: calc(100% / 2 - 12px);
  }
`;function Ot({type:t,setFormActive:r,setModalActive:s}){const o={name:"",carbohydrate:"",protein:"",fat:"",calories:""},a=S();function h(d){return d.charAt(0).toUpperCase()+d.slice(1)}const x=async(d,l)=>{try{await a(ye({diary:h(t),...d})).unwrap(),y.success("You have successfully added your meal!"),r(!1),s(!1)}catch(i){console.log(i),y.error(`Something went wrong! 
 ${i.message}`)}l.resetForm()};return e.jsx(Y,{initialValues:o,validationSchema:De,onSubmit:x,children:({errors:d,touched:l,isSubmitting:i,isValid:c})=>e.jsxs(Dt,{children:[e.jsxs(kt,{children:[e.jsxs(vt,{children:[e.jsx(C,{type:"text",name:"name",placeholder:"The name of the product or dish",autoComplete:"off",className:l.name?d.name?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"name"})]}),e.jsxs(Qt,{children:[e.jsx(C,{type:"number",name:"carbohydrate",placeholder:"Carbonoh.",autoComplete:"off",className:l.carbohydrate?d.carbohydrate?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"carbohydrate"})]}),e.jsxs(Bt,{children:[e.jsx(C,{type:"number",name:"protein",placeholder:"Protein",autoComplete:"off",className:l.protein?d.protein?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"protein"})]}),e.jsxs(It,{children:[e.jsxs(he,{children:[e.jsx(C,{type:"number",name:"fat",placeholder:"Fat",autoComplete:"off",className:l.fat?d.fat?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"fat"})]}),e.jsxs(Pt,{children:[e.jsx(C,{type:"number",name:"calories",placeholder:"Calories",autoComplete:"off",className:l.calories?d.calories?"input-error":"input-success":"input-normal"}),e.jsx(R,{component:"p",name:"calories"})]})]})]}),e.jsxs(Et,{children:[e.jsx(Wt,{type:"submit",disabled:!c||i,children:"Confirm"}),e.jsx(Nt,{type:"button",onClick:()=>{r(!1),s(!1)},children:e.jsx(_,{children:"Cancel"})})]})]})})}function Gt({modalData:t,setModalData:r}){const{type:s}=t,o=be(),a=S(),{meals:h}=A(M),[x,d]=b.useState(!1),l=async i=>{try{await a(Ae(i)).unwrap(),y.success("You have successfully removed your meal!")}catch(c){y.error(`Something went wrong! 
 ${c}`)}};return e.jsx(e.Fragment,{children:e.jsx(ee,{modalActive:!!t,setModalActive:r,children:e.jsxs(bt,{children:[e.jsx(K,{children:"Record your meal"}),e.jsxs(At,{children:[s==="breakfast"&&e.jsx("img",{srcSet:`${B} 1x, ${te} 2x`,src:B,alt:"Breakfast"}),s==="lunch"&&e.jsx("img",{srcSet:`${I} 1x, ${ne} 2x`,src:I,alt:"Lunch"}),s==="dinner"&&e.jsx("img",{srcSet:`${P} 1x, ${ie} 2x`,src:P,alt:"Dinner"}),s==="snack"&&e.jsx("img",{srcSet:`${E} 1x, ${re} 2x`,src:E,alt:"Snack"}),e.jsx(Ft,{children:s})]}),x||h[`${s}`]&&e.jsx(Ct,{children:h[`${s}`].map(i=>e.jsxs(Rt,{children:[e.jsx(St,{children:i.name}),e.jsxs(le,{children:[e.jsxs(j,{children:[e.jsx(p,{children:"Carbonohidrates:"}),e.jsx(m,{children:i.carbohydrate})]}),e.jsxs(j,{children:[e.jsx(p,{children:"Protein:"}),e.jsx(m,{children:i.protein})]}),e.jsxs(j,{children:[e.jsx(p,{children:"Fat:"}),e.jsx(m,{children:i.fat})]}),e.jsxs(j,{children:[e.jsx(p,{children:"Calories:"}),e.jsx(m,{children:i.calories})]}),e.jsx(U,{type:"button",onClick:()=>{l({id:i.id,diary:s})},children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${f}#trash`})})})]})]},o))}),!x&&e.jsx(V,{children:e.jsxs(de,{onClick:()=>d(!0),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${f}#add`})}),e.jsx(xe,{children:"Add more"})]})}),x&&e.jsx(Ot,{setFormActive:d,setModalActive:r,type:s}),e.jsx(V,{})]})})})}function Kt({data:t,meals:r}){const[s,o]=b.useState(!1);return e.jsxs(e.Fragment,{children:[e.jsxs(ft,{children:[e.jsxs(wt,{children:[t.mealType==="breakfast"&&e.jsx("img",{srcSet:`${B} 1x, ${te} 2x`,src:B,alt:"Breakfast"}),t.mealType==="lunch"&&e.jsx("img",{srcSet:`${I} 1x, ${ne} 2x`,src:I,alt:"Lunch"}),t.mealType==="dinner"&&e.jsx("img",{srcSet:`${P} 1x, ${ie} 2x`,src:P,alt:"Dinner"}),t.mealType==="snack"&&e.jsx("img",{srcSet:`${E} 1x, ${re} 2x`,src:E,alt:"Snack"}),e.jsx(yt,{children:t.mealType})]}),r[`${t.mealType}`].length===0?e.jsxs(de,{onClick:()=>o({type:t.mealType,meals:r}),children:[e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${f}#add`})}),e.jsx(xe,{children:"Record your meal"})]}):e.jsxs(le,{children:[e.jsxs(j,{children:[e.jsx(p,{children:"Carbonohidrates:"}),e.jsx(m,{children:t.carbohydrate})]}),e.jsxs(j,{children:[e.jsx(p,{children:"Protein:"}),e.jsx(m,{children:t.protein})]}),e.jsxs(j,{children:[e.jsx(p,{children:"Fat:"}),e.jsx(m,{children:t.fat})]}),e.jsx(U,{type:"button",onClick:()=>o({type:t.mealType,meals:r}),children:e.jsx(u,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${f}#trash`})})})]})]}),e.jsx(Gt,{modalData:s,setModalData:o})]})}function Tt(){S();const{meals:t}=A(M),r=rt(t);return e.jsxs(ut,{children:[e.jsxs(gt,{children:[e.jsx(D,{children:"Diary"}),e.jsx(z,{to:"/diary",children:e.jsx(L,{children:"See more"})})]}),e.jsx(jt,{children:r.map((s,o)=>e.jsx(Kt,{data:s,meals:t},o))})]})}function Ut(){const t=A($),[r,s]=b.useState([]),o=ke(r,4);return b.useEffect(()=>{t&&ve().then(a=>{s(a)}).catch(a=>console.log(a))},[t]),e.jsxs(Qe,{children:[e.jsx(D,{children:"Recommended food"}),r.length===0?e.jsx(X,{}):e.jsx(Be,{children:o.map(a=>e.jsxs(Ie,{children:[e.jsx("img",{src:a.img,alt:a.name,width:"46px"}),e.jsxs("div",{children:[e.jsx(Pe,{children:a.name}),e.jsxs(Ee,{children:[e.jsx(p,{children:a.amount}),e.jsxs(m,{children:[a.calories," calories"]})]})]})]},a._id))}),e.jsxs(z,{to:"/recommended-food",children:[e.jsx(L,{children:"See more"}),e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${f}#arrow-right`})})]})]})}function Vt(){const t=S(),r=A($);b.useEffect(()=>{r&&t(Fe())},[r,t]),b.useEffect(()=>{r&&t(Ce())},[t]);const[s,o]=b.useState(!1);return s?document.body.style.overflow="hidden":document.body.style.overflow="auto",e.jsx(We,{children:e.jsxs(Re,{children:[e.jsxs(Ne,{children:[e.jsx(K,{children:"Today"}),e.jsxs(z,{to:"/dashboard",children:[e.jsx(L,{children:"On the way to the goal"}),e.jsx(u,{width:"16px",height:"16px",children:e.jsx("use",{href:`${f}#arrow-right`})})]})]}),e.jsxs(Oe,{children:[e.jsx(Te,{}),e.jsx(it,{modalActive:s,setModalActive:o}),e.jsx(mt,{}),e.jsx(Tt,{}),e.jsx(Ut,{})]})]})})}export{Vt as default};
