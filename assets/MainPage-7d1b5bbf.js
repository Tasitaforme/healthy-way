import{s as o,d as u,e as I,j as e,f as y,u as k,g as ae,h as ne,_ as j,i as ie,r as A,k as oe,m as Z,o as M,p as V,a as ce,q as le,C as de,t as xe,n as K}from"./index-60d419bc.js";import{a as b,W as E,I as w,b as D,S as W,c as C,d as m,B as R,H as Y,e as f,f as H,g as he,h as J,i as pe,j as U,k as G}from"./Components.styled-ccaac164.js";import{a as me,h as ue,b as Q,d as ge,f as je,F as N,c as B}from"./Formik.styled-2ca59d87.js";import{M as fe,s as X,b as we,l as ye,d as Ae,a as be}from"./snack@2x-4e0b8304.js";import{C as $,A as _,D as ee}from"./index-7daa4d4e.js";import{g as Fe,a as Ce,R as Re,F as Se,b as De,c as ke,d as Pe}from"./getRecommendedFood-f2098ed3.js";import{d as Qe,S as te,e as Be}from"./SignInForm.styled-907a1f85.js";import"./SignInPage.styled-6182dad8.js";const Ie=o.div`
  display: flex;
  flex-wrap: nowrap;
  align-items: center;
  justify-content: space-between;
  margin-top: 16px;
  margin-bottom: 20px;

  @media screen and (min-width: 834px) {
    margin-top: 24px;
    margin-bottom: 24px;
  }
  @media screen and (min-width: 1440px) {
    margin-top: 20px;
    margin-bottom: 16px;
  }
`,ve=o.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  margin-bottom: 80px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    flex-wrap: wrap;
    justify-content: space-between;
    column-gap: 20px;
    margin-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 54px;
  }
`,Oe=o.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 328px;
  }
`,Ee=o.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  @media screen and (min-width: 834px) {
    row-gap: 32px;
  }
  & li {
    display: flex;
    flex-direction: row;
    align-items: center;
    column-gap: 20px;
  }
`;function Ue(){const{baseWater:t,BMR:r}=u(I),i=t!==null?t*1e3:0;return e.jsxs(Oe,{children:[e.jsx(b,{children:"Daily goal"}),e.jsx(E,{children:e.jsxs(Ee,{children:[e.jsxs("li",{children:[e.jsx(w,{width:"80px",height:"80px",stroke:"var(--green)",children:e.jsx("use",{href:`${y}#bubble`})}),e.jsxs("div",{children:[e.jsx(D,{color:"var(--grey-light)",children:"Calories"}),e.jsx(W,{children:r||0})]})]}),e.jsxs("li",{children:[e.jsx(w,{width:"80px",height:"80px",stroke:"var(--violet)",children:e.jsx("use",{href:`${y}#water`})}),e.jsxs("div",{children:[e.jsx(D,{color:"var(--grey-light)",children:"Water"}),e.jsxs(C,{children:[e.jsx(W,{children:i||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]})]})]})]})})]})}const Ge=o.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 444px;
  }
`,Ne=o(E)`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 20px;

  @media screen and (min-width: 834px) {
    column-gap: 40px;
  }
`,Ke=o(D)`
  margin-top: 13px;
`,We=o.div`
  display: flex;
  flex-direction: column;
  row-gap: 8px;
  margin-top: 12px;
  margin-bottom: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 20px;
  }
`,Te=o.div`
  width: 80px;
  height: 192px;
  padding: 8px;
  border-radius: 20px;
  background-color: var(--black-primary);
  border: 1px solid var(--grey);
  display: flex;
  flex-direction: column;
  justify-content: flex-end;
`,qe=o.p`
  position: absolute;
  top: ${t=>t.$top};
  left: 50%;
  transform: translateX(-50%);

  color: ${t=>t.$color};

  text-align: center;
  font-size: 16px;
  font-weight: 600;
  line-height: 1.38;
`,ze=o.div`
  position: relative;

  background-color: var(--violet);
  border-radius: 20px;
  height: ${t=>t.height||"5%"};
`,Le=o.div`
  flex-grow: 1;
  display: flex;
  flex-direction: column;

  & > button:nth-child(1) {
    align-self: flex-end;
  }
`,Ze=o.div`
  margin: 16px 12px 22px;

  @media screen and (min-width: 400px) {
    margin: 24px 40px;
    max-width: 338px;
  }
`,Me=o(R)`
  background-color: transparent;
  color: var(--grey-light);
  width: 100%;
  margin-top: 8px;
  padding-bottom: 0;
  &:hover {
    box-shadow: none;
  }
`,Ve=o(me)`
  @media screen and (min-width: 834px) {
    max-width: 212px;
    margin: 0 auto;
  }
`,Ye=o(ue)`
  margin-top: 24px;
`,He=o(Q)`
  & + p {
    margin-top: 12px;
  }
  & + button {
    margin-top: 32px;
  }
`,Je=o(R)`
  width: 100%;
`,Xe=ge().shape({water:je().min(100,"A quantity of water must be at least 100 ml").max(5e3,"A quantity of water must be at least 5000 ml").required("A quantity of water is required").typeError("A quantity must be a number")});function $e({handleUpdate:t,setModalActive:r}){const i=a=>{t(a)};return e.jsxs(Ze,{children:[e.jsx(Y,{children:"Add water intake"}),e.jsx(N,{initialValues:{water:""},validationSchema:Xe,onSubmit:i,children:({errors:a,touched:s,isSubmitting:c,isValid:d,dirty:p})=>e.jsxs(Ve,{children:[e.jsx(Ye,{children:e.jsx(f,{children:"How much water"})}),e.jsx(He,{type:"text",name:"water",placeholder:"Enter milliliters",autoComplete:"off",className:s.water?a.water?"input-error":"input-success":"input-normal"}),e.jsx(B,{component:"p",name:"water"}),e.jsx(Je,{type:"submit",disabled:!d||c||!p,children:e.jsx(H,{children:"Confirm"})})]})}),e.jsx(Me,{type:"button",onClick:()=>r(!1),children:e.jsx(he,{children:"Cancel"})})]})}const se=t=>t.water;function _e({modalActive:t,setModalActive:r}){const i=k(),{water:a=0,error:s}=u(se),c=u(ae),d=c!==null?c*1e3:0,p=d-a,x=Math.max(0,p),n=Math.round(a*100/d),l=async g=>{r(!1);try{await i(ne({water:+g.water})).unwrap(),j.success("Added to the total amount of water drunk for today!")}catch(F){s&&j.error(`${s}`),j.error(`${F}`)}},h=async()=>{try{await i(ie()).unwrap(),j.success("You have successfully remove daily water!")}catch(g){j.error(`${g}`)}};return e.jsxs(Ge,{children:[e.jsx(b,{children:"Water"}),e.jsxs(Ne,{children:[e.jsx(Te,{children:e.jsx(ze,{height:`${n}%`,children:e.jsxs(qe,{$color:n<84?"var(--violet)":"var(--black-primary)",$top:n<84?"-28px":"28px",children:[n,"%"]})})}),e.jsxs(Le,{children:[e.jsx(J,{type:"button",onClick:()=>h(),children:e.jsx(w,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${y}#trash`})})}),e.jsx(Ke,{children:"Water consumption"}),e.jsxs(We,{children:[e.jsxs(C,{children:[e.jsx(pe,{children:a||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]}),e.jsxs(C,{children:[e.jsx(m,{children:"left:"}),e.jsx(f,{children:x||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]})]}),e.jsxs(R,{type:"button",onClick:()=>r(!0),children:[e.jsx(w,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#add`})}),e.jsx(H,{children:"Add water intake"})]})]})]}),t&&e.jsx(fe,{modalActive:t,setModalActive:r,children:e.jsx($e,{handleUpdate:l,setModalActive:r})})]})}const et=t=>{function r(a){const s={calories:0,carbohydrate:0,protein:0,fat:0};return a.forEach(({calories:c,carbohydrate:d,protein:p,fat:x})=>{s.calories+=c,s.carbohydrate+=d,s.protein+=p,s.fat+=x}),{calories:s.calories,carbohydrate:s.carbohydrate,protein:s.protein,fat:s.fat}}return Object.keys(t).reduce((a,s)=>(Array.isArray(t[s])&&a.push({mealType:s,...r(t[s])}),a),[])},tt=(t={})=>{const r={calories:0,carbohydrate:0,protein:0,fat:0};return Object.values(t).forEach(i=>{Array.isArray(i)&&i.forEach(({calories:a,carbohydrate:s,protein:c,fat:d})=>{r.calories+=a,r.carbohydrate+=s,r.protein+=c,r.fat+=d})}),r},st=o.li`
  min-width: 100%;
  @media screen and (min-width: 1440px) {
    min-width: 560px;
  }
`,rt=o(E)`
  display: flex;
  flex-direction: column;
  row-gap: 12px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 40px;
    padding: 36px 40px;
  }
`,at=o.ul``;$.register(_);const nt=({calories:t,baseCalories:r})=>{const i=A.useMemo(()=>t>=r?100:Math.round(t/r*100),[t,r]),a=t>=r?"red":"rgba(69, 255, 188, 1)",s=t>=r?[0]:[50],c={datasets:[{data:[i,100-i],backgroundColor:[a,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:s,cutout:"80%"}]},d={id:"textCenter",beforeDatasetsDraw(x){const{ctx:n}=x,l=x.getDatasetMeta(0).data[0].x,h=x.getDatasetMeta(0).data[0].y;n.save(),n.font="32px Poppins",n.fillStyle="#ffffff",n.textAlign="center",n.textBaseline="middle",n.fillText(t,l,h-12),n.font="14px sans-serif",n.fillStyle="#B6B6B6",n.fillText("calories",l,h+15)}},p={id:"backgroundCircle",beforeDatasetsDraw(x){const{ctx:n}=x;n.save();const l=x.getDatasetMeta(0).data[0].x,h=x.getDatasetMeta(0).data[0].y,g=x.getDatasetMeta(0).data[0].innerRadius,F=x.getDatasetMeta(0).data[0].outerRadius,S=F-g,P=Math.PI/180;n.beginPath(),n.lineWidth=S,n.strokeStyle="rgba(41, 41, 40, 1)",n.arc(l,h,F-S/2,0,P*360,!1),n.stroke()}};return e.jsx("div",{className:"doughnut",style:{height:168,width:168},children:e.jsx(ee,{plugins:[d,p],data:c})})},it=nt,O=o.li`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,ot=o.ul`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,ct=o(O)`
  &:not(:last-child) {
    margin-bottom: 12px;
  }
`;$.register(_);const lt=({title:t,arcColor:r,nutrient:i,baseNutrient:a=0})=>{const s=A.useMemo(()=>i>=a?100:Math.round(i/a*100),[a,i]),c=i>=a?[0]:[50],d={datasets:[{data:[s,100-s],backgroundColor:[r,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:c,cutout:"80%"}]},p={id:"textCenter",beforeDatasetsDraw(n){const{ctx:l}=n,h=n.getDatasetMeta(0).data[0].x,g=n.getDatasetMeta(0).data[0].y;l.save(),l.font="14px Poppins",l.fillStyle="#B6B6B6",l.textAlign="center",l.textBaseline="middle",l.fillText(`${s}%`,h,g)}},x={id:"backgroundCircle",beforeDatasetsDraw(n){const{ctx:l}=n;l.save();const h=n.getDatasetMeta(0).data[0].x,g=n.getDatasetMeta(0).data[0].y,F=n.getDatasetMeta(0).data[0].innerRadius,S=n.getDatasetMeta(0).data[0].outerRadius,P=S-F,re=Math.PI/180;l.beginPath(),l.lineWidth=P,l.strokeStyle="rgba(41, 41, 40, 1)",l.arc(h,g,S-P/2,0,re*360,!1),l.stroke()}};return e.jsxs(ct,{children:[e.jsx("div",{className:"doughnut",style:{height:48,width:48},children:e.jsx(ee,{plugins:[p,x],data:d})}),e.jsxs("div",{children:[e.jsx(D,{children:t}),e.jsxs(ot,{children:[e.jsxs(O,{children:[e.jsx(m,{children:"Goal:"}),e.jsx(f,{children:a||0})]}),e.jsxs(O,{children:[e.jsx(m,{children:"left:"}),e.jsx(f,{children:i>=a?0:a-i})]})]})]})]})},v=lt;function dt(){const t=k(),{fat:r,protein:i,carbohydrate:a,BMR:s}=u(I),{meals:c,isLoading:d}=u(X),{fat:p,protein:x,carbohydrate:n,calories:l}=tt(c);A.useEffect(()=>{t(oe()).unwrap()},[p,x,n,l]);const h=["Carbonohidrates","Protein","Fat"];return e.jsxs(st,{children:[e.jsx(b,{children:"Food"}),d?e.jsx(Z,{}):e.jsxs(rt,{children:[e.jsx(it,{calories:l,baseCalories:s}),e.jsxs(at,{children:[e.jsx(v,{title:h[0],arcColor:"#FFC4F7",nutrient:n,baseNutrient:a}),e.jsx(v,{title:h[1],arcColor:"#FFF3B7",nutrient:x,baseNutrient:i}),e.jsx(v,{title:h[2],arcColor:"#B6B6B6",nutrient:p,baseNutrient:r})]})]})]})}const xt=o.li`
  width: 100%;

  @media screen and (min-width: 1440px) {
    width: 558px;
  }
`,ht=o.div`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  column-gap: 16px;
  justify-content: flex-start;
  align-items: center;

  margin-bottom: 16px;
`;o(w)`
  width: 36px;
  height: 36px;
  /* fill: none;
  stroke: none; */
  display: inline-block;

  stroke-width: 0;
  stroke: currentColor;
  fill: currentColor;
`;const pt=o.ul`
  display: flex;
  flex-direction: column;
  row-gap: 28px;
`,T="data:image/png;base64,",q="data:image/png;base64,",z="data:image/png;base64,",L="data:image/png;base64,",mt=o.li`
  display: flex;
  flex-direction: column;
  row-gap: 28px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
  }
`,ut=o.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-right: 8px;
`,gt=o(D)`
  text-transform: capitalize;
  width: 100px;
`,jt=o.div`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  column-gap: 20px;
  row-gap: 12px;
  align-items: center;
  justify-content: flex-start;

  & div:nth-child(1) {
    width: 164px;
  }
  & div:nth-child(2) {
    width: 92px;
  }
  & div:nth-child(2) {
    width: 66px;
  }
`;function ft({data:t,meals:r}){return e.jsxs(mt,{children:[e.jsxs(ut,{children:[t.mealType==="breakfast"&&e.jsx("img",{srcSet:`${T} 1x, ${we} 2x`,src:T,alt:"Breakfast"}),t.mealType==="lunch"&&e.jsx("img",{srcSet:`${q} 1x, ${ye} 2x`,src:q,alt:"Lunch"}),t.mealType==="dinner"&&e.jsx("img",{srcSet:`${z} 1x, ${Ae} 2x`,src:z,alt:"Dinner"}),t.mealType==="snack"&&e.jsx("img",{srcSet:`${L} 1x, ${be} 2x`,src:L,alt:"Snack"}),e.jsx(gt,{children:t.mealType})]}),r[`${t.mealType}`]==[]?e.jsx("div",{children:e.jsx("p",{children:"Record your meal"})}):e.jsxs(jt,{children:[e.jsxs(C,{children:[e.jsx(m,{children:"Carbonohidrates:"}),e.jsx(f,{children:t.carbohydrate})]}),e.jsxs(C,{children:[e.jsx(m,{children:"Protein:"}),e.jsx(f,{children:t.protein})]}),e.jsxs(C,{children:[e.jsx(m,{children:"Fat:"}),e.jsx(f,{children:t.fat})]}),e.jsx(J,{type:"button",onClick:()=>console.log("ТУУУ"),children:e.jsx(w,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${y}#trash`})})})]})]})}function wt(){k();const{meals:t,firstLoad:r}=u(X),i=et(t);return e.jsxs(xt,{children:[e.jsxs(ht,{children:[e.jsx(b,{children:"Diary"}),e.jsx(U,{to:"/diary",children:e.jsx(G,{children:"See more"})})]}),e.jsx(pt,{children:i.map((a,s)=>e.jsx(ft,{data:a,meals:t},s))})]})}function yt(){const t=u(M),[r,i]=A.useState([]),a=Fe(r,4);return A.useEffect(()=>{t&&Ce().then(s=>{i(s)}).catch(s=>console.log(s))},[t]),e.jsxs(Re,{children:[e.jsx(b,{children:"Recommended food"}),r.length===0?e.jsx(Z,{}):e.jsx(Se,{children:a.map(s=>e.jsxs(De,{children:[e.jsx("img",{src:s.img,alt:s.name,width:"46px"}),e.jsxs("div",{children:[e.jsx(ke,{children:s.name}),e.jsxs(Pe,{children:[e.jsx(m,{children:s.amount}),e.jsxs(f,{children:[s.calories," calories"]})]})]})]},s._id))}),e.jsxs(U,{to:"/recommended-food",children:[e.jsx(G,{children:"See more"}),e.jsx(w,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#arrow-right`})})]})]})}const At=async t=>{try{const{data:r}=await V.put("/api/auth/change-password",t);return r}catch(r){return r.response.data}};function bt(){const{email:t}=u(I),r={password:"",newPassword:""},i=async(a,s)=>{try{await At({email:t,...a}),j.success("Your password has been successfully changed!")}catch(c){j.error(`Something went wrong! 
 ${c.message}`)}s.resetForm()};return e.jsxs("section",{style:{paddingTop:"24px",paddingBottom:"24px"},children:[e.jsx(b,{style:{marginBottom:"12px"},children:"Change password"}),e.jsx(N,{initialValues:r,validationSchema:Qe,onSubmit:i,children:({errors:a,touched:s,isSubmitting:c,isValid:d})=>e.jsxs(te,{children:[e.jsx(Q,{type:"password",name:"password",placeholder:"Password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(B,{component:"p",name:"password"}),e.jsx(Q,{type:"password",name:"newPassword",placeholder:"New password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(B,{component:"p",name:"newPassword"}),e.jsx(R,{type:"submit",disabled:!d||c,children:"Update Password"})]})})]})}const Ft=async t=>{try{const{data:r}=await V.delete("/api/auth/delete",{data:t});return r}catch(r){return r.response.data}};function Ct(){k();const t=ce(),{email:r}=u(I),i=async(a,s)=>{try{await Ft({email:r,...a}),j.success("You have successfully delete your account!"),t("/welcome")}catch(c){j.error(`Something went wrong! 
 ${c.message}`)}s.resetForm()};return e.jsxs("section",{style:{paddingTop:"24px",paddingBottom:"24px"},children:[e.jsx(b,{style:{marginBottom:"12px",color:"var(--error-color)"},children:"Delete account"}),e.jsx(m,{style:{marginBottom:"12px"},children:"Once you delete your account, there is no going back. Please be certain."}),e.jsx(N,{initialValues:{password:""},validationSchema:Be,onSubmit:i,children:({errors:a,touched:s,isSubmitting:c,isValid:d})=>e.jsxs(te,{children:[e.jsx(Q,{type:"password",name:"password",placeholder:"Password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(B,{component:"p",name:"password"}),e.jsx(R,{type:"submit",disabled:!d||c,children:"Delete your account"})]})})]})}function vt(){const t=k(),r=u(M),{water:i}=u(se);A.useEffect(()=>{r&&t(le())},[r,i]);const[a,s]=A.useState(!1);a?document.body.style.overflow="hidden":document.body.style.overflow="auto";const c=async()=>{try{await t(xe()).unwrap(),K.success("You have successfully logged out!")}catch(d){K.error(`Something went wrong! 
 ${d.message}`)}};return e.jsx("main",{children:e.jsxs(de,{children:[e.jsxs(Ie,{children:[e.jsx(Y,{children:"Today"}),e.jsxs(U,{to:"/dashboard",children:[e.jsx(G,{children:"On the way to the goal"}),e.jsx(w,{width:"16px",height:"16px",children:e.jsx("use",{href:`${y}#arrow-right`})})]})]}),e.jsxs(ve,{children:[e.jsx(Ue,{}),e.jsx(_e,{modalActive:a,setModalActive:s}),e.jsx(dt,{}),e.jsx(wt,{}),e.jsx(yt,{})]}),e.jsx(R,{type:"submit",onClick:()=>c(),children:"Sign out"}),e.jsx(bt,{}),e.jsx(Ct,{})]})})}export{vt as default};
