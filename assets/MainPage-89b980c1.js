import{s as i,d as u,e as B,j as e,f as b,r as w,g as ae,u as D,h as ne,i as ie,_ as f,k as oe,m as ce,o as Z,p as V,q as M,a as le,t as de,C as xe,v as he,n as K}from"./index-696ca15b.js";import{a as A,W as O,I as y,b as k,S as T,c as C,d as m,B as R,H as Y,e as j,f as H,g as pe,h as J,i as me,j as N,k as U}from"./Components.styled-910bfb1a.js";import{c as ue,h as ge,a as P,d as fe,f as je,F as G,b as Q}from"./Formik.styled-c50314a8.js";import{C as X,A as $,D as _}from"./index-b5ebf52d.js";import{b as we,l as ye,d as be,s as Ae}from"./snack@2x-c4dbc989.js";import{g as Fe,a as Ce,R as Re,F as Se,b as ke,c as De,d as ve}from"./getRecommendedFood-6556e4d5.js";import{c as Pe,S as ee,d as Qe}from"./SignInForm.styled-34d7394d.js";import"./SignInPage.styled-04f7fdc5.js";const Be=i.div`
  display: flex;
  flex-wrap: nowrap;
  align-items: center;
  justify-content: space-between;
  margin-top: 16px;
  margin-bottom: 20px;

  @media screen and (min-width: 834px) {
    margin-top: 24px;
    margin-bottom: 24px;
  }
  @media screen and (min-width: 1440px) {
    margin-top: 20px;
    margin-bottom: 16px;
  }
`,Ie=i.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  margin-bottom: 80px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    flex-wrap: wrap;
    justify-content: space-between;
    column-gap: 20px;
    margin-bottom: 40px;
  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 54px;
  }
`,Ee=i.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 328px;
  }
`,Oe=i.ul`
  display: flex;
  flex-direction: column;
  row-gap: 20px;

  @media screen and (min-width: 834px) {
    row-gap: 32px;
  }
  & li {
    display: flex;
    flex-direction: row;
    align-items: center;
    column-gap: 20px;
  }
`;function Ne(){const{baseWater:t,BMR:r}=u(B),o=t!==null?t*1e3:0;return e.jsxs(Ee,{children:[e.jsx(A,{children:"Daily goal"}),e.jsx(O,{children:e.jsxs(Oe,{children:[e.jsxs("li",{children:[e.jsx(y,{width:"80px",height:"80px",stroke:"var(--green)",children:e.jsx("use",{href:`${b}#bubble`})}),e.jsxs("div",{children:[e.jsx(k,{color:"var(--grey-light)",children:"Calories"}),e.jsx(T,{children:r||0})]})]}),e.jsxs("li",{children:[e.jsx(y,{width:"80px",height:"80px",stroke:"var(--violet)",children:e.jsx("use",{href:`${b}#water`})}),e.jsxs("div",{children:[e.jsx(k,{color:"var(--grey-light)",children:"Water"}),e.jsxs(C,{children:[e.jsx(T,{children:o||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]})]})]})]})})]})}const Ue=i.li`
  @media screen and (min-width: 834px) {
    min-width: 380px;
  }
  @media screen and (min-width: 1440px) {
    min-width: 444px;
  }
`,Ge=i(O)`
  width: 100%;
  display: flex;
  flex-direction: row;
  column-gap: 20px;

  @media screen and (min-width: 834px) {
    column-gap: 40px;
  }
`,Ke=i(k)`
  margin-top: 13px;
`,Te=i.div`
  display: flex;
  flex-direction: column;
  row-gap: 8px;
  margin-top: 12px;
  margin-bottom: 16px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 20px;
  }
`,We=i.div`
  width: 80px;
  height: 192px;
  padding: 8px;
  border-radius: 20px;
  background-color: var(--black-primary);
  border: 1px solid var(--grey);
  display: flex;
  flex-direction: column;
  justify-content: flex-end;
`,qe=i.p`
  position: absolute;
  top: ${t=>t.$top};
  left: 50%;
  transform: translateX(-50%);

  color: ${t=>t.$color};

  text-align: center;
  font-size: 16px;
  font-weight: 600;
  line-height: 1.38;
`,Le=i.div`
  position: relative;

  background-color: var(--violet);
  border-radius: 20px;
  height: ${t=>t.height||"5%"};
`,ze=i.div`
  flex-grow: 1;
  display: flex;
  flex-direction: column;

  & > button:nth-child(1) {
    align-self: flex-end;
  }
`,Ze=i.div`
  margin: 16px 12px 22px;

  @media screen and (min-width: 400px) {
    margin: 24px 40px;
    max-width: 338px;
  }
`,Ve=i(R)`
  background-color: transparent;
  color: var(--grey-light);
  width: 100%;
  margin-top: 8px;
  padding-bottom: 0;
  &:hover {
    box-shadow: none;
  }
`,Me=i(ue)`
  @media screen and (min-width: 834px) {
    max-width: 212px;
    margin: 0 auto;
  }
`,Ye=i(ge)`
  margin-top: 24px;
`,He=i(P)`
  & + p {
    margin-top: 12px;
  }
  & + button {
    margin-top: 32px;
  }
`,Je=i(R)`
  width: 100%;
`,Xe=fe().shape({water:je().min(100,"A quantity of water must be at least 100 ml").max(5e3,"A quantity of water must be at least 5000 ml").required("A quantity of water is required").typeError("A quantity must be a number")});function $e({handleUpdate:t,setModalActive:r}){const o=a=>{t(a)};return e.jsxs(Ze,{children:[e.jsx(Y,{children:"Add water intake"}),e.jsx(G,{initialValues:{water:""},validationSchema:Xe,onSubmit:o,children:({errors:a,touched:s,isSubmitting:c,isValid:d,dirty:p})=>e.jsxs(Me,{children:[e.jsx(Ye,{children:e.jsx(j,{children:"How much water"})}),e.jsx(He,{type:"text",name:"water",placeholder:"Enter milliliters",autoComplete:"off",className:s.water?a.water?"input-error":"input-success":"input-normal"}),e.jsx(Q,{component:"p",name:"water"}),e.jsx(Je,{type:"submit",disabled:!d||c||!p,children:e.jsx(H,{children:"Confirm"})})]})}),e.jsx(Ve,{type:"button",onClick:()=>r(!1),children:e.jsx(pe,{children:"Cancel"})})]})}const _e=i.div`
  height: 100vh;
  width: 100vw;
  background-color: rgba(5, 5, 5, 0.8);
  margin: 0 auto;

  position: fixed;
  top: 0;
  left: 0;

  display: flex;
  align-items: flex-start;
  justify-content: center;
  pointer-events: none;

  transition: opacity var(--transition-dur-and-fun);
  opacity: 0;

  &.active {
    opacity: 1;
    pointer-events: all;
  }
`,et=i.div`
  position: relative;
  margin-top: 116px;
  margin-left: 10px;
  margin-right: 10px;

  background-color: var(--black-secondary);
  border-radius: 12px;
  opacity: 1;

  transform: scale(0);
  transition: transform var(--transition-dur-and-fun);

  max-height: calc(100% - 20px);
  overflow-y: auto;

  &.active {
    transform: scale(1);
  }
  @media screen and (min-width: 834px) {
    margin-top: 212px;
    margin-left: auto;
    margin-right: auto;
  }
`,tt=document.querySelector("#root_modal");function rt({modalActive:t,setModalActive:r,children:o}){return t?document.body.style.overflow="hidden":document.body.style.overflow="auto",w.useEffect(()=>{const a=s=>{s.code==="Escape"&&r(!1)};return window.addEventListener("keydown",a),()=>{window.removeEventListener("keydown",a)}},[r]),ae.createPortal(e.jsx(_e,{className:t?"active":"",onClick:()=>r(!1),children:e.jsx(et,{className:t?"active":"",onClick:a=>a.stopPropagation(),children:o})}),tt)}const te=t=>t.water;function st({modalActive:t,setModalActive:r}){const o=D(),{water:a=0,error:s}=u(te),c=u(ne),d=c!==null?c*1e3:0,p=d-a,x=Math.max(0,p),n=Math.round(a*100/d),l=async g=>{r(!1);try{await o(ie({water:+g.water})).unwrap(),f.success("Added to the total amount of water drunk for today!")}catch(F){s&&f.error(`${s}`),f.error(`${F}`)}},h=async()=>{try{await o(oe()).unwrap(),f.success("You have successfully remove daily water!")}catch(g){f.error(`${g}`)}};return e.jsxs(Ue,{children:[e.jsx(A,{children:"Water"}),e.jsxs(Ge,{children:[e.jsx(We,{children:e.jsx(Le,{height:`${n}%`,children:e.jsxs(qe,{$color:n<84?"var(--violet)":"var(--black-primary)",$top:n<84?"-28px":"28px",children:[n,"%"]})})}),e.jsxs(ze,{children:[e.jsx(J,{type:"button",onClick:()=>h(),children:e.jsx(y,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${b}#trash`})})}),e.jsx(Ke,{children:"Water consumption"}),e.jsxs(Te,{children:[e.jsxs(C,{children:[e.jsx(me,{children:a||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]}),e.jsxs(C,{children:[e.jsx(m,{children:"left:"}),e.jsx(j,{children:x||0}),e.jsx(m,{color:"var(--grey-light)",children:"ml"})]})]}),e.jsxs(R,{type:"button",onClick:()=>r(!0),children:[e.jsx(y,{width:"16px",height:"16px",children:e.jsx("use",{href:`${b}#add`})}),e.jsx(H,{children:"Add water intake"})]})]})]}),t&&e.jsx(rt,{modalActive:t,setModalActive:r,children:e.jsx($e,{handleUpdate:l,setModalActive:r})})]})}const re=t=>t.diary,at=t=>{function r(a){const s={calories:0,carbohydrate:0,protein:0,fat:0};return a.forEach(({calories:c,carbohydrate:d,protein:p,fat:x})=>{s.calories+=c,s.carbohydrate+=d,s.protein+=p,s.fat+=x}),{calories:s.calories,carbohydrate:s.carbohydrate,protein:s.protein,fat:s.fat}}return Object.keys(t).map(a=>({mealType:a,...r(t[a])}))},nt=t=>{const r={calories:0,carbohydrate:0,protein:0,fat:0};return Object.values(t).forEach(o=>{o.forEach(({calories:a,carbohydrate:s,protein:c,fat:d})=>{r.calories+=a,r.carbohydrate+=s,r.protein+=c,r.fat+=d})}),r},it=i.li`
  min-width: 100%;
  @media screen and (min-width: 1440px) {
    min-width: 560px;
  }
`,ot=i(O)`
  display: flex;
  flex-direction: column;
  row-gap: 12px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
    column-gap: 40px;
    padding: 36px 40px;
  }
`,ct=i.ul``;X.register($);const lt=({calories:t,baseCalories:r})=>{const o=w.useMemo(()=>t>=r?100:Math.round(t/r*100),[t,r]),a=t>=r?"red":"rgba(69, 255, 188, 1)",s=t>=r?[0]:[50],c={datasets:[{data:[o,100-o],backgroundColor:[a,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:s,cutout:"80%"}]},d={id:"textCenter",beforeDatasetsDraw(x){const{ctx:n}=x,l=x.getDatasetMeta(0).data[0].x,h=x.getDatasetMeta(0).data[0].y;n.save(),n.font="32px Poppins",n.fillStyle="#ffffff",n.textAlign="center",n.textBaseline="middle",n.fillText(t,l,h-12),n.font="14px sans-serif",n.fillStyle="#B6B6B6",n.fillText("calories",l,h+15)}},p={id:"backgroundCircle",beforeDatasetsDraw(x){const{ctx:n}=x;n.save();const l=x.getDatasetMeta(0).data[0].x,h=x.getDatasetMeta(0).data[0].y,g=x.getDatasetMeta(0).data[0].innerRadius,F=x.getDatasetMeta(0).data[0].outerRadius,S=F-g,v=Math.PI/180;n.beginPath(),n.lineWidth=S,n.strokeStyle="rgba(41, 41, 40, 1)",n.arc(l,h,F-S/2,0,v*360,!1),n.stroke()}};return e.jsx("div",{className:"doughnut",style:{height:168,width:168},children:e.jsx(_,{plugins:[d,p],data:c})})},dt=lt,E=i.li`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,xt=i.ul`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
`,ht=i(E)`
  &:not(:last-child) {
    margin-bottom: 12px;
  }
`;X.register($);const pt=({title:t,arcColor:r,nutrient:o,baseNutrient:a=0})=>{const s=w.useMemo(()=>o>=a?100:Math.round(o/a*100),[a,o]),c=o>=a?[0]:[50],d={datasets:[{data:[s,100-s],backgroundColor:[r,"rgba(41, 41, 40, 1)"],borderColor:["rgba(69, 255, 188, 0)"],borderRadius:c,cutout:"80%"}]},p={id:"textCenter",beforeDatasetsDraw(n){const{ctx:l}=n,h=n.getDatasetMeta(0).data[0].x,g=n.getDatasetMeta(0).data[0].y;l.save(),l.font="14px Poppins",l.fillStyle="#B6B6B6",l.textAlign="center",l.textBaseline="middle",l.fillText(`${s}%`,h,g)}},x={id:"backgroundCircle",beforeDatasetsDraw(n){const{ctx:l}=n;l.save();const h=n.getDatasetMeta(0).data[0].x,g=n.getDatasetMeta(0).data[0].y,F=n.getDatasetMeta(0).data[0].innerRadius,S=n.getDatasetMeta(0).data[0].outerRadius,v=S-F,se=Math.PI/180;l.beginPath(),l.lineWidth=v,l.strokeStyle="rgba(41, 41, 40, 1)",l.arc(h,g,S-v/2,0,se*360,!1),l.stroke()}};return e.jsxs(ht,{children:[e.jsx("div",{className:"doughnut",style:{height:48,width:48},children:e.jsx(_,{plugins:[p,x],data:d})}),e.jsxs("div",{children:[e.jsx(k,{children:t}),e.jsxs(xt,{children:[e.jsxs(E,{children:[e.jsx(m,{children:"Goal:"}),e.jsx(j,{children:a||0})]}),e.jsxs(E,{children:[e.jsx(m,{children:"left:"}),e.jsx(j,{children:o>=a?0:a-o})]})]})]})]})},I=pt;function mt(){const t=D(),{fat:r,protein:o,carbohydrate:a,BMR:s}=u(B),{meals:c,isLoading:d}=u(re),{fat:p,protein:x,carbohydrate:n,calories:l}=nt(c);w.useEffect(()=>{t(ce()).unwrap()},[p,x,n,l]);const h=["Carbonohidrates","Protein","Fat"];return e.jsxs(it,{children:[e.jsx(A,{children:"Food"}),d?e.jsx(Z,{}):e.jsxs(ot,{children:[e.jsx(dt,{calories:l,baseCalories:s}),e.jsxs(ct,{children:[e.jsx(I,{title:h[0],arcColor:"#FFC4F7",nutrient:n,baseNutrient:a}),e.jsx(I,{title:h[1],arcColor:"#FFF3B7",nutrient:x,baseNutrient:o}),e.jsx(I,{title:h[2],arcColor:"#B6B6B6",nutrient:p,baseNutrient:r})]})]})]})}const ut=i.li`
  width: 100%;

  @media screen and (min-width: 1440px) {
    width: 558px;
  }
`,gt=i.div`
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  column-gap: 16px;
  justify-content: flex-start;
  align-items: center;

  margin-bottom: 16px;
`;i(y)`
  width: 36px;
  height: 36px;
  /* fill: none;
  stroke: none; */
  display: inline-block;

  stroke-width: 0;
  stroke: currentColor;
  fill: currentColor;
`;const ft=i.ul`
  display: flex;
  flex-direction: column;
  row-gap: 28px;
`,W="data:image/png;base64,",q="data:image/png;base64,",L="data:image/png;base64,",z="data:image/png;base64,",jt=i.li`
  display: flex;
  flex-direction: column;
  row-gap: 28px;

  @media screen and (min-width: 834px) {
    flex-direction: row;
  }
`,wt=i.div`
  display: flex;
  flex-direction: row;
  column-gap: 12px;
  align-items: center;
  justify-content: flex-start;
  margin-right: 8px;
`,yt=i(k)`
  text-transform: capitalize;
  width: 100px;
`,bt=i.div`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  column-gap: 20px;
  row-gap: 12px;
  align-items: center;
  justify-content: flex-start;

  & div:nth-child(1) {
    width: 164px;
  }
  & div:nth-child(2) {
    width: 92px;
  }
  & div:nth-child(2) {
    width: 66px;
  }
`;function At({data:t,meals:r}){return e.jsxs(jt,{children:[e.jsxs(wt,{children:[t.mealType==="breakfast"&&e.jsx("img",{srcSet:`${W} 1x, ${we} 2x`,src:W,alt:"Breakfast"}),t.mealType==="lunch"&&e.jsx("img",{srcSet:`${q} 1x, ${ye} 2x`,src:q,alt:"Lunch"}),t.mealType==="dinner"&&e.jsx("img",{srcSet:`${L} 1x, ${be} 2x`,src:L,alt:"Dinner"}),t.mealType==="snack"&&e.jsx("img",{srcSet:`${z} 1x, ${Ae} 2x`,src:z,alt:"Snack"}),e.jsx(yt,{children:t.mealType})]}),r[`${t.mealType}`]==[]?e.jsx("div",{children:e.jsx("p",{children:"Record your meal"})}):e.jsxs(bt,{children:[e.jsxs(C,{children:[e.jsx(m,{children:"Carbonohidrates:"}),e.jsx(j,{children:t.carbohydrate})]}),e.jsxs(C,{children:[e.jsx(m,{children:"Protein:"}),e.jsx(j,{children:t.protein})]}),e.jsxs(C,{children:[e.jsx(m,{children:"Fat:"}),e.jsx(j,{children:t.fat})]}),e.jsx(J,{type:"button",onClick:()=>console.log("ТУУУ"),children:e.jsx(y,{width:"20px",height:"20px",stroke:"var(--green-light)",children:e.jsx("use",{href:`${b}#trash`})})})]})]})}function Ft(){D();const{meals:t,firstLoad:r}=u(re),o=at(t);return e.jsxs(ut,{children:[e.jsxs(gt,{children:[e.jsx(A,{children:"Diary"}),e.jsx(N,{to:"/diary",children:e.jsx(U,{children:"See more"})})]}),e.jsx(ft,{children:o.map((a,s)=>e.jsx(At,{data:a,meals:t},s))})]})}function Ct(){const t=u(V),[r,o]=w.useState([]),a=Fe(r,4);return w.useEffect(()=>{t&&Ce().then(s=>{o(s)}).catch(s=>console.log(s))},[t]),e.jsxs(Re,{children:[e.jsx(A,{children:"Recommended food"}),r.length===0?e.jsx(Z,{}):e.jsx(Se,{children:a.map(s=>e.jsxs(ke,{children:[e.jsx("img",{src:s.img,alt:s.name,width:"46px"}),e.jsxs("div",{children:[e.jsx(De,{children:s.name}),e.jsxs(ve,{children:[e.jsx(m,{children:s.amount}),e.jsxs(j,{children:[s.calories," calories"]})]})]})]},s._id))}),e.jsxs(N,{to:"/recommended-food",children:[e.jsx(U,{children:"See more"}),e.jsx(y,{width:"16px",height:"16px",children:e.jsx("use",{href:`${b}#arrow-right`})})]})]})}const Rt=async t=>{try{const{data:r}=await M.put("/api/auth/change-password",t);return r}catch(r){return r.response.data}};function St(){const{email:t}=u(B),r={password:"",newPassword:""},o=async(a,s)=>{try{await Rt({email:t,...a}),f.success("Your password has been successfully changed!")}catch(c){f.error(`Something went wrong! 
 ${c.message}`)}s.resetForm()};return e.jsxs("section",{style:{paddingTop:"24px",paddingBottom:"24px"},children:[e.jsx(A,{style:{marginBottom:"12px"},children:"Change password"}),e.jsx(G,{initialValues:r,validationSchema:Pe,onSubmit:o,children:({errors:a,touched:s,isSubmitting:c,isValid:d})=>e.jsxs(ee,{children:[e.jsx(P,{type:"password",name:"password",placeholder:"Password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(Q,{component:"p",name:"password"}),e.jsx(P,{type:"password",name:"newPassword",placeholder:"New password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(Q,{component:"p",name:"newPassword"}),e.jsx(R,{type:"submit",disabled:!d||c,children:"Update Password"})]})})]})}const kt=async t=>{try{const{data:r}=await M.delete("/api/auth/delete",{data:t});return r}catch(r){return r.response.data}};function Dt(){D();const t=le(),{email:r}=u(B),o=async(a,s)=>{try{await kt({email:r,...a}),f.success("You have successfully delete your account!"),t("/welcome")}catch(c){f.error(`Something went wrong! 
 ${c.message}`)}s.resetForm()};return e.jsxs("section",{style:{paddingTop:"24px",paddingBottom:"24px"},children:[e.jsx(A,{style:{marginBottom:"12px",color:"var(--error-color)"},children:"Delete account"}),e.jsx(m,{style:{marginBottom:"12px"},children:"Once you delete your account, there is no going back. Please be certain."}),e.jsx(G,{initialValues:{password:""},validationSchema:Qe,onSubmit:o,children:({errors:a,touched:s,isSubmitting:c,isValid:d})=>e.jsxs(ee,{children:[e.jsx(P,{type:"password",name:"password",placeholder:"Password",autoComplete:"off",className:s.quantity?a.quantity?"input-error":"input-success":"input-normal"}),e.jsx(Q,{component:"p",name:"password"}),e.jsx(R,{type:"submit",disabled:!d||c,children:"Delete your account"})]})})]})}function Ut(){const t=D(),r=u(V),{water:o}=u(te);w.useEffect(()=>{r&&t(de())},[r,o]);const[a,s]=w.useState(!1);a?document.body.style.overflow="hidden":document.body.style.overflow="auto";const c=async()=>{try{await t(he()).unwrap(),K.success("You have successfully logged out!")}catch(d){K.error(`Something went wrong! 
 ${d.message}`)}};return e.jsx("main",{children:e.jsxs(xe,{children:[e.jsxs(Be,{children:[e.jsx(Y,{children:"Today"}),e.jsxs(N,{to:"/dashboard",children:[e.jsx(U,{children:"On the way to the goal"}),e.jsx(y,{width:"16px",height:"16px",children:e.jsx("use",{href:`${b}#arrow-right`})})]})]}),e.jsxs(Ie,{children:[e.jsx(Ne,{}),e.jsx(st,{modalActive:a,setModalActive:s}),e.jsx(mt,{}),e.jsx(Ft,{}),e.jsx(Ct,{})]}),e.jsx(R,{type:"submit",onClick:()=>c(),children:"Sign out"}),e.jsx(St,{}),e.jsx(Dt,{})]})})}export{Ut as default};
